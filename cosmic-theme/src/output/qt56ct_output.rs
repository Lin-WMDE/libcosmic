...
"#,
    ...
    fn apply_ct(ct: &str, is_dark: bool) -> Result<(), OutputError> {
        ...
        if old_version < Theme::COSMIC_QT_VERSION {
            ...
            // Use the WMDE icon theme so Qt apps match GTK and the WMDE apps
            ...
            // Claim color_scheme_path while it still points at a file we wrote: Wmde* now,
            // or the legacy Cosmic* name older builds used. A scheme the user picked
            // themselves in qt5ct/qt6ct is left alone.
            let old_color_scheme_path = ini
                .get("Appearance", "color_scheme_path")
                .unwrap_or_else(|| "WmdePlease".to_owned());
            if old_color_scheme_path.contains("Cosmic") || old_color_scheme_path.contains("Wmde") {
                ini.setstr(
                    "Appearance",
                    "color_scheme_path",
                    color_scheme_path.to_str(),
    ...
    /// Gets a path like `~/.config/qt6ct/colors/WmdeDark.conf`
    ///
